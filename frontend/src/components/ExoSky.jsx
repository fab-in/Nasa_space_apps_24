...
import { BASE_LAYER, BLOOM_LAYER, BLOOM_PARAMS, OVERLAY_LAYER } from "../config/renderConfig.jsx";
...
const ExoSky = ({ra, dec, dist}) => {
  ...
  let renderer, camera, scene, orbit, baseComposer, bloomComposer, overlayComposer;
  ...
  useEffect(() => {
    const initThree = () => {
      ...
      const material = new THREE.MeshBasicMaterial({ color: 0xffffff, opacity: 1, transparent: false });
    ...
    const initRenderPipeline = () => {
      // Set up renderer
      renderer = new THREE.WebGLRenderer({ antialias: true, canvas: canvasRef.current, logarithmicDepthBuffer: true });
  ...
  return <canvas ref={canvasRef} id="canvas" style={{ width: '100%', height: '100%' }}></canvas>;
};
